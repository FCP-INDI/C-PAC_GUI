...
const regressorsInPipelineKey = ['nuisance_corrections', '2-nuisance_regression', 'Regressors']
...
class Regressor extends PureComponent {
  ...
  render() {
    ...
    return (
      <Accordion
        key={`${i}-${name}`}
        expanded={regressor.getIn([key, 'enabled'], false)}
      >
        <AccordionSummary
            classes={{
              root: classes.header,
        ...
        >
          <Switch
            name={`nuisance_corrections.2-nuisance_regression.Regressors.${i}.${key}.enabled`}
      ...
      <ListItem button key={i} onClick={((regi) => () => handlers.handleEdit(regi))(i)}>
        { regressorName ?
          <ListItemText
            ...
            secondary={regressorFormula}
            style={ { padding: '0 115px 0 0' } }
          /> : 
          <ListItemText
            primary={regressorFormula}
            style={ { padding: '0 115px 0 0' } }
          />
        }
...
class NuisanceRegression extends PureComponent {
  ...
  handleNew = () => {
    ...
    regressors = regressors.push(fromJS({
      ...original.toJS(),
      name: `Regressor ${regressors.size}`
    }));
    console.log(regressors.toJS());
  ...
  handleDuplicate = (regi) => {
    const { classes, configuration, onChange } = this.props
    const regressors =
      configuration.getIn(['Regressors']).push(
        configuration.getIn([...regressorsInPipelineKey, regi])
      )
