...
/** Motion Estimate Filter component to afford "USER-DEFINED" filters */
class MotionEstimateFilter extends PureComponent {
  static propTypes = {
  ...
  /** Helper to pass key, value to updater.
   * @param {Immtable.List} change updated list of filters
   * @param {function} onChange
   */
  handleChange = ((change, onChange) => {
    onChange({
      target: {
        name: ['functional_preproc', 'motion_estimates_and_correction', 'motion_estimate_filter'], 
        value: change
      }
    });
  });
  ...
  handleEdit = (fil) => {
    this.setState({ editFilter: fil; });
  };

  handleNew = () => {
    const { classes, configuration, onChange } = this.props
    let filters = configuration.getIn(['Filters']);
    filters = filters.push(fromJS({
      Name: `Filter ${filters.size + 1}`
    }));

    this.handleChange(filters, onChange);
    this.setState({ editFilter: filters.size - 1 });
  };
...
export default withStyles(MotionEstimateFilter.styles)(MotionEstimateFilter);
